...
将多个 Proxy Agent 组成一个分组，每个节点可设置优先级。分组内支持两种选择策略：

| 组内策略 | 说明 |
|----------|------|
| **Smart** | 智能选择 - 在组内自动选择解锁状态最佳的节点 |
| **Fallback** | 故障转移 - 按组内优先级顺序尝试，当前节点失败时自动切换到下一个 |
...
├─────────────────────────────────────────────────┤
│  节点A (优先级1)  ←─┐                            │
│  节点B (优先级2)  ←─┼── Fallback: 按优先级降级    │
│  节点C (优先级3)  ←─┘                            │
│                                                 │
│  或者                                           │
│                                                 │
│  节点A (解锁✓)   ←─┐                            │
│  节点B (解锁✗)   ←─┼── Smart: 选最佳解锁状态     │
│  节点C (解锁✓)   ←─┘                            │
└─────────────────────────────────────────────────┘
```

**示例场景**：
- **Fallback 模式**：节点A > 节点B > 节点C，优先使用A，A故障则用B，B故障则用C
